...
/**
 * EditorBridge — click-to-annotate when loaded inside the Site Editor iframe.
 * Only activates when embedded in a parent window (the editor).
 */
export function EditorBridge() {
  ...
  // Check if we're inside the editor iframe
  useEffect(() => {
    const hasEditorParent = window.parent !== window;
    setIsEditorContext(hasEditorParent);

    if (hasEditorParent) {
      // Tell the editor we're ready
      window.parent.postMessage({ type: 'SITE_EDITOR_READY' }, '*');
    }
  }, []);

  // Listen for control messages from parent editor
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      // Execute scripts sent from the editor (e.g. element highlighting)
      if (event.data?.type === 'SITE_EDITOR_EXECUTE_SCRIPT') {
        try {
          new Function(event.data.script)();
        } catch (e) {
          console.warn('[EditorBridge] Script execution error:', e);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (target.closest('[data-editor-bridge]')) return;
  ...
  // Submit annotation to parent editor
  const submitAnnotation = useCallback(() => {
    if (!annotationInput || !noteText.trim()) return;
      ...
      {annotationInput && (
        <>
          ...
          <div
            ...
            style={{
              position: 'fixed',
              top: Math.min(annotationInput.rect.bottom + 12, window.innerHeight - 160),
              left: Math.max(16, Math.min(annotationInput.rect.left, window.innerWidth - 296)),
              width: '280px',
              backgroundColor: '#fff',
              borderRadius: '12px',
          ...
          >
            ...
            <div style={{ marginTop: '8px', fontSize: '11px', color: '#9ca3af' }}>
              Enter to submit &bull; Esc to cancel
            </div>
          </div>
        </>
      )}
